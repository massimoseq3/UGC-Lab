...
interface WaveformProps {
  ...
  // decoded (see `useWaveformPeaks` in audio.ts).
  peaks: number[] | null
...
export default function Waveform({ peaks, progress, onSeek, className = '' }: WaveformProps) {
  const bars = peaks ?? []
      ...
      {(bars.length > 0 ? bars : new Array(56).fill(REST)).map((peak, i) => (
        <span
          ...
          className={`h-full min-w-[2px] flex-1 origin-center rounded-full transition-colors duration-150 ${
            bars.length > 0 && i < played ? 'bg-voice-300' : 'bg-ink/15'
          }`}
